...
fixed_img = cv2.resize(bin_img, (target_module_count, target_module_count), interpolation=cv2.INTER_NEAREST)
clean_img = cv2.resize(fixed_img, (target_module_count*6, target_module_count*6), interpolation=cv2.INTER_NEAREST)
    ...
    for r in range(size):
        for c in range(size):
            if mask[r, c] == 0:
                ...
                new_img[r*module_size + module_size//2, c*module_size + module_size//2] = center_val  # 填中間
